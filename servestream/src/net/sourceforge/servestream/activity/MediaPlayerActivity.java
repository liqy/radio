...
public class MediaPlayerActivity extends SherlockFragmentActivity implements
		MusicUtils.Defs, OnSharedPreferenceChangeListener, CoverViewListener,
		LoadingDialogListener, SleepTimerDialogListener {
	...
	public void onSharedPreferenceChanged(SharedPreferences sharedPreferences,
		...
		if (key.equals(PreferenceConstants.WAKELOCK)) {
			if (sharedPreferences
					.getBoolean(PreferenceConstants.WAKELOCK, true)) {
				getWindow().addFlags(
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case EFFECTS_PANEL: {
			try {
				...
				i.putExtra("android.media.extra.AUDIO_SESSION",
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		if (Build.VERSION.SDK_INT >= 9) {
			...
			// before API level 9 because it relies on the getAudioSessionId
			// method,
			...
			if (menu.findItem(EFFECTS_PANEL) == null) {
				...
				if (getPackageManager().resolveActivity(i, 0) != null) {
					menu.add(0, EFFECTS_PANEL, 0, R.string.list_menu_effects)
	...
	private void setSleepTimer(int pos) {
		...
		try {
			...
			if (pos == MediaPlaybackService.SLEEP_TIMER_OFF) {
				...
				showToast(getString(R.string.sleep_timer_on_notif) + " "
						+ makeTimeString(pos));
			}
		} catch (RemoteException e) {
		}
	...
	private String makeTimeString(int pos) {
		...
		if (pos == MediaPlaybackService.SLEEP_TIMER_OFF) {
			...
			minuteText = getResources().getString(R.string.hours,
					String.valueOf(pos / 60));
		} else if (pos % 60 == 0) {
			minuteText = getResources().getString(R.string.hour);
		} else {
			minuteText = getResources().getString(R.string.minutes,
					String.valueOf(pos));
		}
	...
	private void setRepeatButtonImage() {
		...
		try {
			...
			case MediaPlaybackService.REPEAT_ALL:
				mRepeatButton
						.setImageResource(R.drawable.btn_player_repeat_checked);
				break;
			case MediaPlaybackService.REPEAT_CURRENT:
				mRepeatButton
						.setImageResource(R.drawable.btn_player_repeat_one_checked);
				break;
			default:
				mRepeatButton
						.setImageResource(R.drawable.btn_player_repeat_normal);
				break;
			}
		} catch (RemoteException ex) {
		}
	...
	private void setShuffleButtonImage() {
		...
		try {
			...
			case MediaPlaybackService.SHUFFLE_NONE:
				mShuffleButton
						.setImageResource(R.drawable.btn_player_shuffle_normal);
				break;
			default:
				mShuffleButton
						.setImageResource(R.drawable.btn_player_shuffle_checked);
				break;
			}
		} catch (RemoteException ex) {
		}
			...
			if (action.equals(MediaPlaybackService.META_CHANGED)) {
				...
				try {
					if (mService != null) {
						...
						mAlbumArtHandler.obtainMessage(REFRESH_ALBUM_ART,
								new IdWrapper(mService.getTrackId()))
								.sendToTarget();
					}
				} catch (RemoteException e) {
				}
	...
	private void updateTrackInfo() {
		...
		try {
			...
			mAlbumArtHandler.obtainMessage(GET_ALBUM_ART,
					new IdWrapper(mService.getTrackId())).sendToTarget();
	...
	public class AlbumArtHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			if (((msg.what == GET_ALBUM_ART && mId != id) || msg.what == REFRESH_ALBUM_ART)
					&& id >= 0) {
	...
	public void showDialog(String tag) {
		...
		if (tag.equals(SLEEP_TIMER_DIALOG)) {
			...
			try {
				newFragment = SleepTimerDialog.newInstance(this,
						mService.getSleepTimerMode());
			} catch (RemoteException e) {
			}
		}
