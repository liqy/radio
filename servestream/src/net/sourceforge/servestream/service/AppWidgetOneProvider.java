...
import net.sourceforge.servestream.activity.HomeActivity;
...
public class AppWidgetOneProvider extends AppWidgetProvider {
	...
	/**
	 * Initialize given widgets to default state, where we launch ServeStream on
	 * default click and hide actions if service not running.
	 */
	private void defaultAppWidget(Context context, int[] appWidgetIds) {
		final RemoteViews views = new RemoteViews(context.getPackageName(),
				R.layout.appwidget_one);
	...
	public void performUpdate(MediaPlaybackService service, int[] appWidgetIds,
		...
		if (what.equals(MediaPlaybackService.PLAYER_CLOSED)) {
			...
			if (trackName == null || trackName.equals(Media.UNKNOWN_STRING)) {
				trackName = res
						.getText(R.string.widget_one_track_info_unavailable);
			}
			...
			Bitmap b = BitmapFactory.decodeStream(service.getResources()
					.openRawResource(R.drawable.albumart_mp_unknown_widget),
					null, opts);
	 ...
	 *            click will launch {@link MediaPlayerActivity}, otherwise we
	 *            launch {@link UriListActivity}.
	 */
